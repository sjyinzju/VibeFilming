...
Phase 3 now implements the model-neutral media request/provider/router/job/binary artifact/preview/QC/repair/timeline boundaries described above. FLUX Direct is the first real image adapter. The ComfyUI bridge now owns versioned API-format template compilation and transport, but ships no active H3 workflow and performs no real video inference. Spark changes, model downloads, serving process management, media-model deployment, LoRA, GPU scheduling, authentication, public deployment, and real video generation remain external/deferred. See `p3_media_runtime.md`, `comfyui_video_integration_plan.md`, and `p3_extension_guide.md`.
